...
export class InfoUser implements OnInit {
  ...
  submit() {
    let confermaChange = prompt(
      'inserisca la sua password per abilitare le modifiche'
    );

    if (this.form.valid && confermaChange == this.user.password) {
      console.log(this.arrayUsers);
      this.submitEM.emit(this.form.value);
      let tmpUsersname = this.arrayUsers.filter(
        (obj: any) =>
          obj.username == this.form.value.username &&
          obj.username != this.user.username
      );

      let tmpEmail = this.arrayUsers.filter(
        (obj: any) =>
          obj.email == this.form.value.email && obj.email != this.user.email
      );
      console.log(tmpUsersname);
      if (
        tmpUsersname.length == 0 &&
        tmpEmail.length == 0 &&
        this.form.value.username.length > 7 &&
        this.form.value.username.includes('.') &&
        this.form.value.email.includes('.') &&
        this.form.value.email.includes('@') &&
        this.form.value.email.length > 4 &&
        this.form.value.password.includes('.') &&
        this.form.value.email.length > 6 &&
  ...
  ngOnInit(): void {
    ...
    if (id == this.userId.toString()) {
      this.user = this.arrayUsers.filter(
        (obj: any) => obj.id == this.userId
      )[0];
    } else {
